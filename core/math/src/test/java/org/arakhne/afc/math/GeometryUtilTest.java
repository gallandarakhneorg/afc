...
*/
	...
	/**
 */
	public void testInterpolateFloatFloatFloatFloatFloat() {
		assertEpsilonEquals(new Point2f(1, 2),
				(Point2f) GeometryUtil.interpolate(1.f, 2.f, 3.f, 4.f, 0.f));
		assertEpsilonEquals(new Point2f(1.5f, 2.5f),
				(Point2f) GeometryUtil.interpolate(1.f, 2.f, 3.f, 4.f, .25f));
		assertEpsilonEquals(new Point2f(2, 3.f),
				(Point2f) GeometryUtil.interpolate(1.f, 2.f, 3.f, 4.f, .5f));
		assertEpsilonEquals(new Point2f(2.5f, 3.5f),
				(Point2f) GeometryUtil.interpolate(1.f, 2.f, 3.f, 4.f, .75f));
		assertEpsilonEquals(new Point2f(3, 4),
				(Point2f) GeometryUtil.interpolate(1.f, 2.f, 3.f, 4.f, 1.f));
	}
	...
	public void testGetCohenSutherlandCodeFloatFloatFloatFloatFloatFloat() {
		
		Point2f[] Test = { new Point2f(1, -1), new Point2f(1, 2),
				new Point2f(6, 2), new Point2f(6, -1), new Point2f(3.6, 2),
				new Point2f(1, 0.54), new Point2f(3.5, -1),
				new Point2f(6, 0.46), new Point2f(3.54, 0.46),
				new Point2f(6, 4), new Point2f(4, 4), new Point2f(1, 4),
				new Point2f(-1, 4), new Point2f(-1, 2),
				new Point2f(-1.04, 0.64), new Point2f(-1, -1),
				new Point2f(-1, -3), new Point2f(1, -3), new Point2f(3, -3),
				new Point2f(6, -3), new Point2f(8, -3), new Point2f(8, -1),
				new Point2f(8.06, 0.42), new Point2f(8, 2), new Point2f(8, 4) };
		int result = 0;
		for(int i = 0 ; i < Test.length ; i++){
			assertEpsilonEquals("i" + i, result, GeometryUtil.getCohenSutherlandCode(Test[i].getX(), Test[i].getY(), 1, -1, 5, 3));
			if(i == 8) result = 8;
			if(i == 11)result = 9;
			if(i == 12)result = 1;
			if(i == 15)result = 5;
	...
	public void testIsUnitVectorFloatFloatFloatFloatFloatFloatFloatFloatFloat() {// TODO
																					// still
																					// remain
																					// problem
																					// here
		...
		Vector3f Test[] = { new Vector3f(-0.72, -0.38, 1.83),
				...
				new Vector3f(-0.47, 1.22, 1.51) };
		for (int i = 0; i < Test.length; i++) {
			assertFalse("True " + i, GeometryUtil.isUnitVector(Test[i].getX(),
					Test[i].getY(), Test[i].getZ()));
			float d = Test[i].getX() * Test[i].getX() + Test[i].getY()
					* Test[i].getY() + Test[i].getZ() * Test[i].getZ();
			Test[i].normalize();
			assertTrue(
					"False i : " + i + "Test :" + Test[i].toString() + "d : "
							+ d,
					GeometryUtil.isUnitVector(Test[i].getX() / d,
							Test[i].getY() / d, Test[i].getZ() / d));
		}
	}

	public void testIsUnitVectorFloatFloatFloatFloat() {
		//Les Points envoyé sont à 1O⁽-6) près :).
		...
		for (int i = 0; i < TestOn.length; i++) {
			assertTrue("i " + i , GeometryUtil.isUnitVector(TestOn[i].getX(), TestOn[i].getY()));
		}
		for (int i = 0; i < TestIn.length; i++) {
			assertFalse("i " + i , GeometryUtil.isUnitVector(TestIn[i].getX(), TestIn[i].getY()));
		}
		for (int i = 0; i < TestOut.length; i++) {
			assertFalse("i " + i, GeometryUtil.isUnitVector(TestOut[i].getX(), TestOut[i].getY()));
		}	
	}
