 ...
 */
package org.arakhne.afc.math.geometry.continuous.object3d.convexhull;
...
import org.arakhne.afc.math.geometry.continuous.object3d.plane.PlaneClassifier;
...
public class HullTriangle3D<T extends Point3f> implements HullObject<T>, PlaneClassifier {
    ...
    public PlanarClassificationType classifies(Tuple3f vec) {
        PlanarClassificationType lc;
        int cmp = MathUtil.epsilonDistanceSign(distanceTo(vec));
        if (cmp < 0)
            lc = PlanarClassificationType.BEHIND;
        else if (cmp > 0)
            lc = PlanarClassificationType.IN_FRONT_OF;
        else
            lc = PlanarClassificationType.COINCIDENT;
        return lc;
    }
    ...
    public PlanarClassificationType classifies(float lx, float ly, float lz, float ux, float uy, float uz) {
        ...
        return (sign<0) ? PlanarClassificationType.BEHIND : PlanarClassificationType.IN_FRONT_OF;
    }
    ...
    public PlanarClassificationType classifies(float x, float y, float z, float radius) {
        ...
        if (cmp==0) return PlanarClassificationType.COINCIDENT; 
        if (d>radius) return PlanarClassificationType.IN_FRONT_OF;
    ...
    public PlanarClassificationType classifies(Plane otherPlane) {
    	...
    	if (distance>0) return PlanarClassificationType.IN_FRONT_OF;
    	return PlanarClassificationType.BEHIND;
    }
