 ...
 * @deprecated see {@link org.arakhne.afc.math.geometry.d2.continuous.RoundRectangle2f}
 */
@Deprecated
public class RoundRectangle2f extends AbstractRectangularShape2f<RoundRectangle2f> {
	...
	public Point2D getClosestPointTo(Point2D p) {
		...
		if (px<rx1+aw) {
			if (py<ry1+ah) {
				return toOldPoint(MathUtil.getClosestPointToSolidEllipse(
						px, py,
						rx1, ry1,
						aw, ah));
			}
	...
	private static Point2D toOldPoint(org.arakhne.afc.math.geometry.d2.Point2D p) {
