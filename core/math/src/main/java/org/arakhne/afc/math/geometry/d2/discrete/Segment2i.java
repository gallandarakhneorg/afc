...
public class Segment2i extends AbstractShape2i<Segment2i> {
	...
	public void set(Shape2i s) {
		if (s instanceof Segment2i) {
			Segment2i g = (Segment2i) s;
	...
	public static Point2i computeFarthestPointTo(int ax, int ay, int bx, int by, int px, int py) {
		...
		if ((v1x*v1x+v1y*v1y) >= (v2x*v2x+v2y*v2y)) {
			return new Point2i(ax, ay);
		}
		return new Point2i(bx, by);
	}
	 ...
	 */
	public boolean clipToRectangle(int rxmin, int rymin, int rxmax, int rymax) {
		int x0 = getX1();
		int y0 = getY1();
		int x1 = getX2();
		int y1 = getY2();
