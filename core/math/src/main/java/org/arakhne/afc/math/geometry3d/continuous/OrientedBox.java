...
import org.arakhne.afc.math.geometry.PointFilterXYIterable;
import org.arakhne.afc.math.geometry.PointFilterXZIterable;
import org.arakhne.afc.math.geometry.PointFilterYZIterable;
 ...
 */
public class OrientedBox extends AbstractCombinableBounds3D implements TranslatableBounds3D, RotatableBounds3D, OrientedCombinableBounds3D {
	 ...
	 * @param vertices
	 */
	public OrientedBox(Tuple3f... vertices) {
		this.isBoundInit = false;
	 ...
	 * @param vertices
	 */
	public OrientedBox(Collection<? extends Tuple3f> vertices) {
		this.isBoundInit = false;
	 ...
	 * @param extent
	 */
	public OrientedBox(Point3f center, Vector3f[] axis, float[] extent) {
		assert (center != null);
		assert (axis != null);
		assert (axis[0] != null);
		assert (axis[1] != null);
	 ...
	 * @param extent
	 */
	public OrientedBox(Point3f center, Vector3f[] axis, Tuple3f extent) {
		assert (center != null);
		assert (axis != null);
		assert (axis[0] != null);
		assert (axis[1] != null);
	...
	public OrientedBox clone() {
		OrientedBox clone = (OrientedBox) super.clone();
	 ...
	 */
	@Override
	public OrientedRectangle2f toBounds2D() {
	 ...
	 */
	@Override
	public OrientedRectangle2f toBounds2D(CoordinateSystem3D system) {
		if (!isInit())
			return new OrientedRectangle2f();
		...
		return new OrientedRectangle2f(points);
	}
	...
	public boolean equals(Object o) {
		...
		if (this == o)
			return true;
		if (o instanceof OrientedBox) {
			OrientedBox s = (OrientedBox) o;
	...
	private void computeVertices() {
		...
		for (int i = 0; i < this.vertices.length; ++i) {
			if (this.vertices[i].getX() < this.aabbLower.getA())
				this.aabbLower.setA(this.vertices[i].getX());
			if (this.vertices[i].getY() < this.aabbLower.getB())
				this.aabbLower.setB(this.vertices[i].getY());
			if (this.vertices[i].getZ() < this.aabbLower.getC())
				this.aabbLower.setC(this.vertices[i].getZ());
			if (this.vertices[i].getX() > this.aabbUpper.getA())
				this.aabbUpper.setA(this.vertices[i].getX());
			if (this.vertices[i].getY() > this.aabbUpper.getB())
				this.aabbUpper.setB(this.vertices[i].getY());
			if (this.vertices[i].getZ() > this.aabbUpper.getC())
				this.aabbUpper.setC(this.vertices[i].getZ());
		}
	...
	private void computeBoundingBoxes(PlaneConstraint constraint, Iterable<? extends Tuple3f> vertexList) {
		...
		if (constraint != null) {
			...
			case OXY:
				...
				OrientedRectangle2f.computeOBRAxis(new PointFilterXYIterable(vertexList), R, S);
			...
			case OXZ:
				...
				OrientedRectangle2f.computeOBRAxis(new PointFilterXZIterable(vertexList), R, S);
			...
			case OYZ:
				...
				OrientedRectangle2f.computeOBRAxis(new PointFilterYZIterable(vertexList), R, S);
	...
	public float getSizeX() {
		...
		if (this.aabbLower == null || this.aabbUpper == null)
			computeVertices();
		return this.aabbUpper.getA() - this.aabbLower.getA();
	}
	...
	public float getSizeY() {
		...
		if (this.aabbLower == null || this.aabbUpper == null)
			computeVertices();
		return this.aabbUpper.getB() - this.aabbLower.getB();
	}
	...
	public float getSizeZ() {
		...
		if (this.aabbLower == null || this.aabbUpper == null)
			computeVertices();
		return this.aabbUpper.getC() - this.aabbLower.getC();
	}
	...
	public Vector3f getSize() {
		...
		if (this.aabbLower == null || this.aabbUpper == null)
			computeVertices();
		return new Vector3f(this.aabbUpper.getA() - this.aabbLower.getA(), this.aabbUpper.getB() - this.aabbLower.getB(), this.aabbUpper.getC() - this.aabbLower.getC());
	}
	...
	private static class MergedBoundList implements Iterable<Tuple3f> {
		 ...
		 */
		public MergedBoundList(Collection<? extends Bounds3D> bounds, OrientedBox obb) {
			this.bounds = bounds;
			this.obb = obb;
		}
