...
public class Segment2f extends AbstractShape2f<Segment2f> {
	...
	public float distanceSquared(Point2D p) {
		return GeometryUtil.distanceSquaredPointToSegment(p.getX(), p.getY(),
				this.ax, this.ay,
				this.bx, this.by);
	}
	...
	public boolean contains(float x, float y) {
		return MathUtil.isEpsilonZero(GeometryUtil.distanceSquaredPointToSegment(x, y,
				this.ax, this.ay,
				this.bx, this.by));
	}
	...
	public Point2f getClosestPointTo(Point2D p) {
		float ratio = GeometryUtil.getPointProjectionFactorOnLine(p.getX(), p.getY(),
				this.ax, this.ay,
				this.bx, this.by);
