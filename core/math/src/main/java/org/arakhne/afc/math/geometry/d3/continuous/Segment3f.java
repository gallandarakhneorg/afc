...
public class Segment3f extends AbstractShape3f<Segment3f> {
	...
	public void transform(Transform3D transformationMatrix) {
		transformationMatrix.transform(this.pivot);
		transformationMatrix.transform(this.d);
	}

	@Override
	public void translate(double dx, double dy, double dz) {
		this.pivot.add(dx, dy, dz);
	}
	...
	public boolean intersects(Triangle3f s) {
		return Triangle3f.intersectsTriangleSegment(
				s.getX1(), s.getY1(), s.getZ1(),
				s.getX2(), s.getY2(), s.getZ2(),
	...
	public boolean intersects(Plane3D<?> p) {
		...
		double s = n.dot(this.d);
		return !MathUtil.isEpsilonZero(s);
	}

	@Override
	public boolean isEmpty() {
		return this.d.lengthSquared() == 0.;
	}
	...
	public Point3D getClosestPointTo(Point3D p) {
		...
		return new Point3f(
				this.pivot.getX() + ratio * this.d.getX(),
				this.pivot.getY() + ratio * this.d.getY(),
				this.pivot.getZ() + ratio * this.d.getZ());
	}
	...
	public void set(Shape3f s) {
		if (s instanceof Segment3f) {
			...
			this.pivot.set(r.getMinX(), r.getMinY(), r.getMinZ());
			this.d.set(r.getSizeX(), r.getSizeY(), r.getSizeZ());
		}
	}
	...
	public boolean equals(Object obj) {
		...
		if (obj instanceof Segment3f) {
			Segment3f s3f = (Segment3f) obj;
