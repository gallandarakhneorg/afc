 ...
 */
public class Sphere extends AbstractCombinableBounds3D implements TranslatableBounds3D, AlignedCombinableBounds3D, RotatableBounds3D, OrientedCombinableBounds3D {
     ...
     * @param radius is the radius of the sphere.
     */
    public Sphere(Tuple3f<?> p, float radius) {
        this.center.set(p);
        this.radius = Math.abs(radius);
    	this.isBoundInit = true;
    }
     ...
     */
	public Sphere(Collection<? extends Sphere> bboxList) {
     ...
     * @param bboxList are the bounding objects used to initialize this bounding sphere
     */
	public Sphere(CombinableBounds3D... bboxList) {
	...
	public String toString() {
    	...
    	s.append(this.center.getA());
    	s.append(',');
    	s.append(this.center.getB());
    	s.append(',');
    	s.append(this.center.getC());
	...
	public void getLowerUpper(Point3f lower, Point3f upper) {
		...
		assert(upper!=null);
		lower.set(
				this.center.getA()-this.radius,
				this.center.getB()-this.radius,
				this.center.getC()-this.radius);
		upper.set(
				this.center.getA()+this.radius,
				this.center.getB()+this.radius,
				this.center.getC()+this.radius);
	...
	public void combine(float ptX, float ptY, float ptZ) {
		if (!this.isBoundInit) {
			...
			float d = MathUtil.distancePointPoint(ptX,ptY,ptZ,this.center.getA(),this.center.getB(),this.center.getC());
			if (d>this.radius) {
				d = (d + this.radius)/2.f;
	...
	protected void combinePoints(boolean isAlreadyInit, Collection<? extends Tuple3f> pointList) {
		if (pointList!=null && !pointList.isEmpty()) {
			if (!isAlreadyInit) {
				...
				float distance;
				for(Tuple3f<?> point : pointList) {
					distance = MathUtil.distancePointPoint(this.center.getA(),this.center.getB(),this.center.getC(),point.getX(),point.getY(),point.getZ());
	...
	public EuclidianPoint3D farestPoint(Point3f reference) {
		...
		if (v.length()==0.) {
			farest.set(this.center.getA()+this.radius, this.center.getB(), this.center.getC());
		}
		else {
			v.normalize();
			v.scale(this.radius);
	...
	public IntersectionType classifies(Plane plane) {
		...
		return (plane.classifies(
				this.center.getA(), this.center.getB(), this.center.getC(), this.radius) == PlanarClassificationType.COINCIDENT)
	...
	public boolean intersects(Plane plane) {
		...
		return plane.intersects(
				this.center.getA(), this.center.getB(), this.center.getC(), this.radius);
	}
