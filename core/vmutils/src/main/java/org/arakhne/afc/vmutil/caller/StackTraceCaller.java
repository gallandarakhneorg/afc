...
public class StackTraceCaller implements Caller {
		...
		try {
			...
			for (var i = 0; i < stackTrace.length; ++i) {
				if (found) {
					...
					final var type = loadClass(stackTrace[i].getClassName());
					if (type != null && Caller.class.isAssignableFrom(type)) {
						j = i + 1;
					} else if (j >= 0) {
						// First occurrence of a class in the stack, after the
	...
	public Class<?> getCallerClass(int level) {
		final var element = getTraceElementAt(level);
		if (element == null) {
			throw new IllegalArgumentException();
		}
		final var type = loadClass(element.getClassName());
		if (type == null) {
			throw new IllegalArgumentException();
		}
		return type;
