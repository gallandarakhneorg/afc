...
public class Path3i extends AbstractShape3i<Path3i>
        implements Path3ai<Path3i, PathElement3i, Point3i, Vector3i, Quaternion4i, AlignedBox3i> {
	...
	private boolean buildLogicalBoundingBox(AlignedBox3i box) {
		if (this.numCoords > 0) {
			...
			var zmin = this.coords[2];
			var xmax = xmin;
			var ymax = ymin;
			var zmax = zmin;
			for (var i = 3; i < this.numCoords; i +=  3) {
	...
	public boolean containsControlPoint(Point3D<?, ?, ?> point) {
		...
		final var px = point.ix();
		final var py = point.iy();
		final var pz = point.iz();
	...
	public Point3i[] toPointArray(Transform3D transform) {
        final var clone = new Point3i[this.numCoords / 2];
		if (transform == null || transform.isIdentity()) {
