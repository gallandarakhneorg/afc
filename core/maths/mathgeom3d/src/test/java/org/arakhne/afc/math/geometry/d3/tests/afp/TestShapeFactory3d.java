...
public interface TestShapeFactory3d<P extends Point3D<? super P, ? super V, ? super Q>,
    ...
    Path3afp<?, ?, P, V, Q, B> createPath();

    MultiShape3afp<?, ?, ?, P, V, Q, B> createMultiShape();

	Q createQuaternion(double x, double y, double z, double w);

	Q createAxisAngle(double x, double y, double z, double angle);

	Transform3D createTransform();

	Plane3afp createPlane(double a, double b, double c, double d);

	PlaneXY3afp createPlaneXY(boolean positive, double z);

	PlaneXZ3afp createPlaneXZ(boolean positive, double y);

	PlaneYZ3afp createPlaneYZ(boolean positive, double x);
