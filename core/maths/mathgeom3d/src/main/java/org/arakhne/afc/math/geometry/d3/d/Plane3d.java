...
public class Plane3d extends AbstractPlane3d<Plane3d> {
	...
	/** Cached pivot point.
	 */
	private transient SoftReference<Point3d> cachedPivot;
	...
	public Plane3d(double normalx, double normaly, double normalz, double pivotx, double pivoty, double pivotz) {
		final var length = Math.sqrt(normalx * normalx + normaly * normaly + normalz * normalz);
	...
	public void setPivot(double x, double y, double z) {
		final var pivot = getGeomFactory().newPoint(x, y, z);
		// a x + b y + c z + d = 0
		// a x + b y + c z = -d
	...
	public Plane3d normalize() {
		final var length = Math.sqrt(this.a * this.a + this.b * this.b + this.c * this.c);
	...
	public Point3d getPivot() {
		var pivot = this.cachedPivot == null ? null : this.cachedPivot.get();
		if (pivot == null) {
	...
	public void translate(double x, double y, double z) {
		final var refPoint = getPivot();
		final var nx = refPoint.getX() + x;
		final var ny = refPoint.getY() + y;
		final var nz = refPoint.getZ() + z;
		setPivot(nx, ny, nz);
	}
