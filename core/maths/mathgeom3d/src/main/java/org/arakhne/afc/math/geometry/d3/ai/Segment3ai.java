...
public interface Segment3ai<
	...
	static void computeClosestPointToPoint(int ax, int ay, int az, int bx, int by, int bz, int px, int py, int pz,
		...
		final var cp = new InnerComputationPoint3ai();
		...
		while (iterator.hasNext()) {
			...
			final var c = Math.abs(pz - cp.iz());
			final var distance = a * a + b * b + c * c;
			if (distance == 0) {
	...
	static double computeClosestPointToSegment(
		...
		final var it1 = new BresenhamLineIterator<>(InnerComputationGeomFactory.SINGLETON, s1x1, s1y1, s1z1, s1x2, s1y2, s1z2);
		final var it2 = new BresenhamLineIterator<>(InnerComputationGeomFactory.SINGLETON, s2x1, s2y1, s2z1, s2x2, s2y2, s2z2);

		final var p1 = new InnerComputationPoint3ai();
		final var p2 = new InnerComputationPoint3ai();
		...
		do {
			...
			while (it1.hasNext()) {
				it1.next(p1);
				var distance = p1.getDistanceSquared(p2);
				if (distance > minDistance) {
					break;
				} else if (distance <= 1) {
					// Special case: may at distance 1 due to no shared pixel,
					// but segments are intersecting. So that distance must be zero.
					final var side1 = computeSideLinePoint(s1x1, s1y1, s1z1, s1x2, s1y2, s1z2, s2x1, s2y1, s2z1);
					final var side2 = computeSideLinePoint(s1x1, s1y1, s1z1, s1x2, s1y2, s1z2, s2x2, s2y2, s2z2);
					if (side1 == -side2) {
						distance = 0;
					}
				}
	...
	static double computeClosestPointToRectangle(int sx1, int sy1, int sz1, int sx2, int sy2, int sz2,
		...
		final var rmaxz = rz + rdepth;
		final var code1 = MathUtil.getCohenSutherlandCode3D(sx1, sy1, sz1, rx, ry, rz, rmaxx, rmaxy, rmaxz);
		final var code2 = MathUtil.getCohenSutherlandCode3D(sx2, sy2, sz2, rx, ry, rz, rmaxx, rmaxy, rmaxz);
		final Point3D<?, ?, ?> tmp1 = new InnerComputationPoint3ai();
		final Point3D<?, ?, ?> tmp2 = new InnerComputationPoint3ai();
		final var zone = AlignedBox3ai.reduceCohenSutherlandZoneAlignedBoxSegment(
	...
	static int computeSideLinePoint(int x1, int y1, int z1, int x2, int y2, int z2, int px, int py, int pz) {
		final var segmentX = x2 - x1;
		final var segmentY = y2 - y1;
		//TODO final var segmentZ = z2 - z1;
		final var targetX = px - x1;
		final var targetY = py - y1;
		//TODO final var targetZ = pz - z1;
		final var side = segmentX * targetY - segmentY * targetX;
	 ...
	 *
	 * <p><a href="../../d2/ai/doc-files/crossing_circle.png"><img src="../../d2/ai/doc-files/crossing_circle.png" width="300" alt=""></a>
	 *
	...
	static int computeCrossingsFromSphere(
		...
		//TODO final var zmin = cz - Math.abs(radius);
		//TODO final var zmax = cz + Math.abs(radius);
	 ...
	 * The red lines represent the up and bottom borders.
	 *
	 * <p><a href="../../d2/ai/doc-files/crossing_segment.png"><img src="../../d2/ai/doc-files/crossing_segment.png" width="300" alt=""></a>
	 *
	 * @param crossings is the initial value for the number of crossings.
	...
	static int computeCrossingsFromSegment(
		...
		if (x0 > xmax && x1 > xmax) {
			...
			if (side1 <= 0 || side2 <= 0) {
				...
				final var n1 = computeCrossingsFromPoint(0, sx1, sy1, sz1, x0, y0, z0, x1, y1, z1, firstIsTop, !firstIsTop);
				final var n2 = computeCrossingsFromPoint(0, sx2, sy2, sz2, x0, y0, z0, x1, y1, z1, !firstIsTop, firstIsTop);
	 ...
	 * The red lines represent the up and bottom borders.
	 *
	 * <p><a href="../../d2/ai/doc-files/crossing_rect.png"><img src="../../d2/ai/doc-files/crossing_rect.png" width="300" alt=""></a>
	 *
	 * @param crossings is the initial value for the number of crossings.
	...
	static int computeCrossingsFromRect(
		...
		if (x0 > rxmax && x1 > rxmax) {
			...
			final var p = new InnerComputationPoint3ai();
	 ...
	 * The red lines represent the up and bottom borders.
	 *
	 * <p><a href="../../d2/ai/doc-files/crossing_point.png"><img src="../../d2/ai/doc-files/crossing_point.png" width="300" alt=""></a>
	 *
	...
	static boolean intersectsSegmentSegment(int x1, int y1, int z1, int x2, int y2, int z2, int x3, int y3, int z3, int x4,
			int y4, int z4) {
		final var side1 = computeSideLinePoint(x1, y1, z1, x2, y2, z2, x3, y3, z3);
		final var side2 = computeSideLinePoint(x1, y1, z1, x2, y2, z2, x4, y4, z4);
		if ((side1 * side2) <= 0) {
	...
	default boolean contains(int x, int y, int z) {
		...
		if (x >= ax && x <= bx && y >= ay && y <= by && z >= az && z <= bz) {
			...
			var minDist = Integer.MAX_VALUE;
			final var p = new InnerComputationPoint3ai();
			final var iterator = new BresenhamLineIterator<>(InnerComputationGeomFactory.SINGLETON, ax, ay, az, bx, by, bz);
			while (iterator.hasNext()) {
				iterator.next(p);
				final var a = Math.abs(x - p.ix());
				final var b = Math.abs(y - p.iy());
				final var c = Math.abs(z - p.iz());
				final var dist = a * a + b * b + c * c;
				if (dist == 0) {
					return true;
				}
				if (dist > minDist) {
	...
	default void transform(Transform3D transform) {
		...
		final var p = getGeomFactory().newPoint(getX1(), getY1(), getZ1());
	...
	default boolean clipToRectangle(int rxmin, int rymin, int rzmin, int rxmax, int rymax, int rzmax) {
		...
		var code2 = MathUtil.getCohenSutherlandCode3D(x1, y1, z0, rxmin, rymin, rzmin, rxmax, rymax, rzmax);
		var accept = false;
		var cont = true;
	...
	class BresenhamLineIterator<P extends Point3D<? super P, ? super V, ? super Q>,
		...
		//TODO private final var zstep;

		private final int deltax;

		private final int deltay;

		//TODO private final var deltaz;
		...
		public BresenhamLineIterator(GeomFactory3D<V, P, Q> factory, int x0, int y0, int z0, int x1, int y1, int z1) {
			...
			var localx0 = x0;
			var localy0 = y0;
			final var localz0 = z0;
			var localx1 = x1;
			var localy1 = y1;
			final var localz1 = z1;
