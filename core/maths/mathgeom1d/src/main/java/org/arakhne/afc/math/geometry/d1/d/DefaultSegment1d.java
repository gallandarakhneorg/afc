...
public class DefaultSegment1d implements Segment1D<Point2d, Vector2d> {
	...
	/** Change the length of the segment. This function does not change the reference points of the segments.
	 *
	...
	public void projectsOnPlane(double positionOnSegment, Point2D<?, ?> position, Vector2D<?, ?> tangent) {
		...
		if (position != null) {
			final var vector2 = new InnerComputationVector2D(vector);
			vector2.scale(positionOnSegment);
	...
	public boolean isFirstPointConnectedTo(Segment1D<?, ?> otherSegment) {
		assert otherSegment != null : AssertMessages.notNullParameter();
