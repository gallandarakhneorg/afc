...
public interface Shape1afp<
	...
	default <TY extends Enum<TY>> TY getType(Class<TY> enumerationType) {
		assert enumerationType != null && enumerationType.equals(Shape1DType.class) : AssertMessages.invalidValue(0);
	...
	default boolean contains(Shape1D<?, ?, ?, ?, ?, ?> shape) {
		...
		final var type = shape.getType(Shape1DType.class);
		assert type != null;
