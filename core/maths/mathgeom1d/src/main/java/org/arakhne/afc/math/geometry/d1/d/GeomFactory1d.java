...
public class GeomFactory1d extends AbstractGeomFactory1D<Vector1d, Point1d>
	...
	public Point1d convertToPoint(Point1D<?, ?, ?> pt) {
		...
		if (pt instanceof Point1d pts) {
			return pts;
		}
		return new Point1d(pt.getSegment(), pt.getX(), pt.getY());
	}

	@Override
	public Point1d convertToPoint(Vector1D<?, ?, ?> vector) {
		assert vector != null : AssertMessages.notNullParameter();
		return new Point1d(vector.getSegment(), vector.getX(), vector.getY());
	}

	@Override
	public Vector1d convertToVector(Point1D<?, ?, ?> pt) {
		assert pt != null : AssertMessages.notNullParameter();
		return new Vector1d(pt.getSegment(), pt.getX(), pt.getY());
	}

	@Override
	public Vector1d convertToVector(Vector1D<?, ?, ?> vector) {
		assert vector != null : AssertMessages.notNullParameter();
		if (vector instanceof Vector1d vec) {
			return vec;
		}
		return new Vector1d(vector.getSegment(), vector.getX(), vector.getY());
	}
