...
public class TestShapeFactory3d implements TestShapeFactory3afp<Point3d, Vector3d, Quaternion4d, AlignedBox3d> {
	
	public static final TestShapeFactory3d SINGLETON = new TestShapeFactory3d();
	
	@Override
	public Segment3afp<?, ?, ?, Point3d, Vector3d, Quaternion4d, AlignedBox3d> createSegment(double x1, double y1, double z1, double x2, double y2, double z2) {
	...
	@Override
	public Path3afp<?, ?, ?, Point3d, Vector3d, Quaternion4d, AlignedBox3d> createPath(PathWindingRule rule) {
		if (rule == null) {
			return new Path3d();
		}
