...
package org.arakhne.afc.math.geometry.base.coordinatesystem;
...
public enum CoordinateSystem3D implements CoordinateSystem {
	...
	XZY_LEFT_HAND {
		...
		public QuaternionComponents toSystem(QuaternionComponents rotation, CoordinateSystem3D targetCoordinateSystem) {
			...
			assert targetCoordinateSystem != null : AssertMessages.notNullParameter(1);
			final var axisangle = Quaternion.computeAxisAngle(rotation.x(), rotation.y(), rotation.z(), rotation.w());
			final var x = axisangle.x();
			var y = axisangle.y();
			var z = axisangle.z();
			var a = axisangle.angle();
			switch (targetCoordinateSystem) {
			case XZY_RIGHT_HAND:
				z = -z;
				a = -a;
		...
		protected double toCoordinateSystem2DAngleFromTransformation(Transform3D mat) {
			...
			final var ptR = new InnerComputationVector3D(1., 0., 0.);
			mat.transform(ptR);
	 ...
	 *
	 * @return {@code true} if z coordinate is up.
	 * @since 18.0
	 */
	@Pure
	 ...
	 *
	 * @return {@code true} if y coordinate is up.
	 * @since 18.0
	 */
	@Pure
	 ...
	 *
	 * @return {@code true} if z coordinate is side.
	 * @since 18.0
	 */
	@Pure
	 ...
	 *
	 * @return {@code true} if y coordinate is side.
	 * @since 18.0
	 */
	@Pure
	...
	public void toSystem(Quaternion<?, ?, ?> rotation, CoordinateSystem3D targetCoordinateSystem) {
		var comps = new QuaternionComponents(rotation.getX(), rotation.getY(), rotation.getZ(), rotation.getW());
	...
	public void toSystem(Transform3D matrix, CoordinateSystem3D targetCoordinateSystem) {
		...
		final var translation = new InnerComputationPoint3D(
				matrix.getTranslationX(), matrix.getTranslationY(), matrix.getTranslationZ());
		final var rotation = matrix.getRotation(InnerComputationGeomFactory3D.SINGLETON);
	...
	public double toCoordinateSystem2D(Quaternion<?, ?, ?> rotation) {
		final var trans = new Transform3D();
	 ...
	 */
	@Pure
	protected abstract double toCoordinateSystem2DAngleFromTransformation(Transform3D matrix);
