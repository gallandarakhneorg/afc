 ...
 */
public class BoundingCapsule extends AbstractCombinableBounds3D implements TranslatableBounds3D, RotatableBounds3D, OrientedCombinableBounds3D {
	 ...
	 */
	public BoundingCapsule(Point3f a, Point3f b, float radius) {
	...
	public Bounds2D toBounds2D(CoordinateSystem3D system) {
		...
		axis[0].normalize();
		axis[1].normalize();
		return new OrientedBoundingRectangle(center2d, axis, extent);
	}
	...
	public Vector3f getSize() {
		return new Vector3f(this.upper.getX() - this.lower.getX(), this.upper.getY() - this.lower.getY(), this.upper.getZ() - this.lower.getZ());
	}
	 ...
	 */
	public boolean intersectsCapsule(BoundingCapsule capsule) {
	...
	public IntersectionType classifies(Point3f c, float r) {
		return IntersectionUtil.classifySphereCapsule(c, r, this.a, this.b, this.radius);
	}
	...
	public IntersectionType classifies(Point3f center, Vector3f[] axis, float[] extent) {
		return IntersectionUtil.classifyOrientedBoxCapsule(center, axis, extent, this.a, this.b, this.radius);
	}
	...
	public IntersectionType classifies(Point3f p) {
		return IntersectionUtil.classifyPointCapsule(p, this.a, this.b, this.radius);
	}

	/**
	 * Compute intersection between an OBB and a capsule
	 */
	private boolean intersectsAlignedBoxCapsule(AlignedBoundingBox aabb) {
	 ...
	 */
	private IntersectionType classifyAlignedBoxCapsule(AlignedBoundingBox aabb) {
		Point3f closestFromA = aabb.nearestPoint(this.a);
		Point3f closestFromB = aabb.nearestPoint(this.b);
		...
		IntersectionType onSphereA = IntersectionUtil.classifiesSolidSphereSolidAlignedBox(this.a, this.radius, aabb.getLower(), aabb.getUpper());
		IntersectionType onSphereB = IntersectionUtil.classifiesSolidSphereSolidAlignedBox(this.b, this.radius, aabb.getLower(), aabb.getUpper());
	...
	public IntersectionType classifies(Point3f l, Point3f u) {
		return classifyAlignedBoxCapsule(new AlignedBoundingBox(l, u));
	}
	...
	public boolean intersects(Point3f l, Point3f u) {
		return intersectsAlignedBoxCapsule(new AlignedBoundingBox(l, u));
	}
	...
	private void computeBoundingCapsule(Iterable<? extends Tuple3f> vertexList) {
		...
		OrientedBoundingBox.computeOBBCenterAxisExtents(vertexList, this.axis[0], this.axis[1], this.axis[2], this.center, this.extent);
	...
	private static class MergedBoundList implements Iterable<Tuple3f> {
		...
		private final BoundingCapsule capsule;
		 ...
		 */
		public MergedBoundList(Collection<? extends Bounds3D> bounds, BoundingCapsule obb) {
			this.bounds = bounds;
			this.capsule = obb;
		}
