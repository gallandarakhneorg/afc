...
public enum CoordinateSystem3D implements CoordinateSystem {
	...
	private float toCoordinateSystem2DAngleFromTransformation(Matrix4f mat) {
		...
		case XYZ_LEFT_HAND:
		case XYZ_RIGHT_HAND:
			return MathUtil.signedAngle(1, 0, ptR.getX(), ptR.getY());
		case XZY_LEFT_HAND:
		case XZY_RIGHT_HAND:
			return MathUtil.signedAngle(ptR.getX(), ptR.getZ(), 1, 0);
		default:
			throw new CoordinateSystemNotFoundException();
		}
	}
	 ...
	 * @since 4.0
	 */
	public Transform2D toCoordinateSystem2D(Matrix4f matrix) {
		// Translation
