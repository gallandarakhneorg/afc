...
image:https://img.shields.io/badge/requires-Java%2025-red.svg?style=flat-square[link=https://www.java.com]
image:https://img.shields.io/maven-central/v/org.arakhne.afc/afc.svg?label=Maven%20Central[link=https://central.sonatype.com/search?q=org.arakhne.afc]
...
See link:http://arakhne.org/java/afc/[Arakhne.org] for details.
...
For example, link:http://arakhne.org/java/afc/apidocs/index.html?org/arakhne/afc/math/geometry/d2/Vector2D.html[Vector2D] provides overloading function for operator `+`. In this way, this operator may be used by typing `v1 + 1` instead of `v1.add(1)`.
...
For AFC version 18.0 or higher:

* Java Development Toolkit (JDK) 25 or higher.

For AFC version 17.0:

* Java Development Toolkit (JDK) 11 or higher.

For AFC before version 17.0:
...
The lastest stable version of AFC is available on link:https://central.sonatype.com/[Maven Central].
...
Please, replace Version `18.0` in the previous snipset by the number of the version you want to use (`18.0` is the first version that is available on Maven Central).
