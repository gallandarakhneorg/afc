 ...
 */
public class PerceptionList1D53D<SB extends CombinableBounds1D5<RoadSegment>, MB extends CombinableBounds1D5<RoadSegment>>
extends PerceptionList1D5<SB,MB,Perception3D> {
	
	/**
	 */
	public PerceptionList1D53D() {
		//
	}

	@Override
	public boolean isSupportedPerceptionType(Class<? extends Perception> type) {
		return type.isAssignableFrom(Perception3D.class);
	}

	@Override
	protected Perception3D createPerception(
			CullingResult1D5<Entity1D5<? extends CombinableBounds1D5<RoadSegment>>> result) {
		return new Perception1D53D(
				result.getFrustum(),
