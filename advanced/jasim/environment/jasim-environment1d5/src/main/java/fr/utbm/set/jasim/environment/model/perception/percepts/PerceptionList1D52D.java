 ...
 */
public class PerceptionList1D52D<SB extends CombinableBounds1D5<RoadSegment>, MB extends CombinableBounds1D5<RoadSegment>>
extends PerceptionList1D5<SB,MB,Perception2D> {
	
	/**
	 */
	public PerceptionList1D52D() {
		//
	}

	@Override
	public boolean isSupportedPerceptionType(Class<? extends Perception> type) {
		return type.isAssignableFrom(Perception2D.class);
	}

	@Override
	protected Perception2D createPerception(
			CullingResult1D5<Entity1D5<? extends CombinableBounds1D5<RoadSegment>>> result) {
		return new Perception1D52D(
				result.getFrustum(),
