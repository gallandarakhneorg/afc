 ...
 */
public class PerceptionList3D3D<SB extends CombinableBounds3D, MB extends CombinableBounds3D> 
extends PerceptionList3D<SB,MB,Perception3D> {
	
	/**
	 */
	public PerceptionList3D3D() {
		//
	}

	@Override
	protected Perception3D createPerception(CullingResult<? extends Entity3D<? extends CombinableBounds3D>> result) {
		return new Perception3D3D(
				result.getFrustum(),
	...
	public boolean isSupportedPerceptionType(Class<? extends Perception> type) {
		return type.isAssignableFrom(Perception3D3D.class);
	}
