 ...
 */
public class BspTreeManipulator3D<DB extends CombinableBounds3D>
extends AbstractTreeManipulator3D<DB,
	 ...
	 */
	public BspTreeManipulator3D(int splittingLevel, PartitionPolicy partitionPolicy, AlignedBoundingBox universe) {
		super(splittingLevel, 
				NonIcosepBinaryPartitionFieldFactory.SINGLETON, 
				partitionPolicy,
				universe);
	}
	 ...
	 */
	@Override
	protected int classifies(DynamicBspTreeNode3D<DB> node, CombinableBounds3D location) {
		int i = BspTreeUtil.classifiesOnCoordinates(
	 ...
	 */
	@Override
	protected DynamicBspTreeNode3D<DB> createAndBindChildNode(
			DynamicBspTreeNode3D<DB> parent, int index,
	 ...
	 */
	@Override
	protected AlignedBoundingBox refineUniverse(DynamicBspTreeNode3D<DB> node, AlignedBoundingBox universe, int region) {
		return BspTreeUtil3D.getSubArea(
				universe, 
				BinaryTreeZone.values()[region], 
				node.getCutCoordinate(),
				node.getCutCoordinateIndex());
	}
