 ...
 */
public class IcosepOctTreeManipulator3D<DB extends CombinableBounds3D>
extends AbstractTreeManipulator3D<DB,
	 ...
	 */
	public IcosepOctTreeManipulator3D(int splittingLevel, PartitionPolicy partitionPolicy, AlignedBoundingBox universe) {
		super(splittingLevel, 
				IcosepOctyPartitionFieldFactory.SINGLETON, 
				partitionPolicy,
				universe);
	}
	 ...
	 */
	@Override
	protected int classifies(DynamicIcosepOctTreeNode3D<DB> node, CombinableBounds3D location) {
		int i = OctTreeUtil.classifiesBoxWithIcosep(
				node.getCutLineX(),
				node.getCutLineY(),
				node.getCutLineZ(),
	 ...
	 */
	@Override
	protected DynamicIcosepOctTreeNode3D<DB> createAndBindChildNode(
			DynamicIcosepOctTreeNode3D<DB> parent, int index,
	 ...
	 */
	@Override
	protected AlignedBoundingBox refineUniverse(DynamicIcosepOctTreeNode3D<DB> node, AlignedBoundingBox universe, int region) {
		return OctTreeUtil3D.getSubArea(
				universe, 
				IcosepOctTreeZone.values()[region], 
