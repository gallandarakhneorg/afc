...
/**
 * This class is used to issue draw calls to a {@code GisCanvas} using a buffer.
 *
 * <p>The {@code GisGraphicsContext} is a wrapper to the {@link GraphicsContext}
 * embedded within the {@link GisCanvas}.
 *
 * <p>The coordinates that are supported by the {@code GisGraphicsContext} are
 * GIS coordinates (not the screen coordinates). Consequently, the
 * {@code GisGraphicsContext} translates any coordinates from the GIS standard
...
public class GisGraphicsContext {
	...
	private final ReadOnlyObjectProperty<Rectangle2afp<?, ?, ?, ?, ?, ?>> mapBounds;

	private final ReadOnlyObjectProperty<Rectangle2d> visibleArea;
	 ...
	 * @param visibleArea the visible part of the map.
	 ...
	 *     system from the "global" GIS coordinate system to/from the "centered" GIS coordinate
	 *     system.
	 */
	@SuppressWarnings("checkstyle:magicnumber")
	public GisGraphicsContext(
			GraphicsContext gc,
			DoubleProperty scale,
			ReadOnlyObjectProperty<Rectangle2afp<?, ?, ?, ?, ?, ?>> mapBounds,
		...
		assert mapBounds != null : AssertMessages.notNullParameter(2);
	...
	/** Replies the map bounds in GIS coordinates.
	 *
	 * @return the map bounds.
	 */
	@Pure
	public Rectangle2afp<?, ?, ?, ?, ?, ?> getMapBounds() {
		return this.mapBounds.get();
	}

	/** Replies the bounds of the visible area in GIS coordinates.
	 *
	...
	/**
	 * Gets the {@code GisCanvas} that the {@code GraphicsContext} is issuing draw
	 * commands to. There is only ever one {@code GisCanvas} for a
	 ...
	 */
	@Pure
	public GisCanvas<?> getCanvas() {
		return (GisCanvas<?>) this.gc.getCanvas();
	}
	...
	/** Transform a GIS x coordinate to its JavaFX equivalent.
	 *
	 * @param x the gis x coordinate.
	 * @return the JavaFX x coordinate.
	 */
	@Pure
	public double gis2fxX(double x) {
		return this.centeringTransform.toCenterX(x) * this.scale.get() + this.canvasWidth.get() / 2.;
	}

	/** Transform a JavaFX x coordinate to its GIS equivalent.
	 *
	 * @param x the gis x coordinate.
	 * @return the JavaFX x coordinate.
	 */
	@Pure
	public double fx2gisX(double x) {
		return this.centeringTransform.toGlobalX((x - this.canvasWidth.get() / 2.) / this.scale.get());
	}

	/** Transform a GIS y coordinate to its JavaFX equivalent.
	 *
	 * @param y the gis y coordinate.
	 * @return the JavaFX y coordinate.
	 */
	@Pure
	public double gis2fxY(double y) {
		return this.centeringTransform.toCenterY(y) * this.scale.get() + this.canvasHeight.get() / 2.;
	}

	/** Transform a JavaFX y coordinate to its GIS equivalent.
	 *
	 * @param y the JavaFX y coordinate.
	 * @return the GIS y coordinate.
	 */
	@Pure
	public double fx2gisY(double y) {
		return this.centeringTransform.toGlobalY((y - this.canvasHeight.get() / 2.) / this.scale.get());
	}

	/** Transform a GIS size (or distance) to its JavaFX equivalent.
	 *
	 * <p>The JavaFX size {@code fx_x} is equal to:
	 * <pre><code>fx_size = size * scale</code></pre>
	 *
	 * @param size the gis size.
	 * @return the JavaFX size.
	 */
	@Pure
	public double gis2fxSize(double size) {
		return size * this.scale.get();
	}

	/** Transform a JavaFX size (or distance) to its GIS equivalent.
	 *
	 * <p>The GIS coordinate {@code gis_x} is equal to:
	 * <pre><code>gis_size = size / scale</code></pre>
	 *
	 * @param size the JavaFX size.
	 * @return the GIS size.
	 */
	@Pure
	public double fx2gisSize(double size) {
		return size / this.scale.get();
	}

	/** Transform a GIS angle to its JavaFX equivalent.
	 *
	 * <p>The JavaFX angle {@code fx_angle} is equal to:
	 * <pre><code>fx_angle = - angle</code></pre>
	 *
	 * @param angle the gis angle.
	 * @return the JavaFX angle.
	 */
	@Pure
	public double gis2fxAngle(double angle) {
		final GisCanvas<?> canvas = getCanvas();
	...
	/** Transform a JavaFX angle to its GIS equivalent.
	 *
	 * <p>The gis angle {@code gis_angle} is equal to:
	 * <pre><code>gis_angle = - angle</code></pre>
	 *
	 * @param angle the JavaFX angle.
	 * @return the gis angle.
	 */
	@Pure
	public double fx2gisAngle(double angle) {
		final GisCanvas<?> canvas = getCanvas();
		if (canvas.isInvertedAxisX() != canvas.isInvertedAxisY()) {
			return -angle;
		}
	...
	public void translate(double x, double y) {
		this.gc.translate(gis2fxX(x), gis2fxY(y));
	}
	...
	public void scale(double x, double y) {
		this.gc.scale(gis2fxSize(x), gis2fxSize(y));
	}
	...
	public void rotate(double radians) {
		this.gc.rotate(gis2fxAngle(Math.toDegrees(radians)));
	}
	 ...
	 */
	@Inline(value = "setLinewidthInPixels(($0).gis2fxSize($1))")
	public void setLineWidthInMeters(double lw) {
		setLineWidthInPixels(gis2fxSize(lw));
	}
	 ...
	 */
	@Inline(value = "fx2gisSize(($0).getLineWidthInPixels())")
	public double getLineWidthInMeters() {
		return fx2gisSize(getLineWidthInPixels());
	}
	 ...
	 */
	@Inline(value = "setMiterLimitInPixels(($0).gis2fxSize($1))")
	public void setMiterLimitInMeters(double ml) {
		setMiterLimitInPixels(gis2fxSize(ml));
	}
	 ...
	 */
	@Inline(value = "fx2gisSize(($0).getMiterLimitInPixels())")
	public double getMiterLimitInMeters() {
		return fx2gisSize(getMiterLimitInPixels());
	}
	...
	public void setLineDashesInMeters(double... dashes) {
		...
		if (dashes == null) {
			...
			for (int i = 0; i < newDashes.length; ++i) {
				newDashes[i] = gis2fxSize(dashes[i]);
			}
		}
	...
	public double[] getLineDashesInMeters() {
		...
		for (int i = 0; i < newDashes.length; ++i) {
			newDashes[i] = gis2fxSize(pdashes[i]);
		}
		return newDashes;
	}
	 ...
	 */
	@Inline(value = "setLineDashOffsetInPixels(($0).gis2fxSize($1))")
	public void setLineDashOffsetInMeters(double dashOffset) {
		setLineDashOffsetInPixels(gis2fxSize(dashOffset));
	}
	 ...
	 */
	@Inline(value = "fx2gisSize(($0).getLineDashOffsetInPixels())")
	public double getLineDashOffsetInMeters() {
		return fx2gisSize(getLineDashOffsetInPixels());
	}
	...
	public void fillText(String text, double x, double y) {
		this.gc.fillText(text, gis2fxX(x), gis2fxY(y));
	}
	...
	public void fillText(String text, double x, double y, double maxWidth) {
		this.gc.fillText(text, gis2fxX(x), gis2fxY(y), gis2fxSize(maxWidth));
	}
	...
	public void strokeText(String text, double x, double y) {
		this.gc.strokeText(text, gis2fxX(x), gis2fxY(y));
	}
	...
	public void strokeText(String text, double x, double y, double maxWidth) {
		this.gc.strokeText(text, gis2fxX(x), gis2fxY(y), gis2fxSize(maxWidth));
	}
	...
	public void moveTo(double x0, double y0) {
		this.gc.moveTo(gis2fxX(x0), gis2fxY(y0));
	}
	...
	public void lineTo(double x1, double y1) {
		this.gc.lineTo(gis2fxX(x1), gis2fxY(y1));
	}
	...
	public void quadraticCurveTo(double xc, double yc, double x1, double y1) {
		this.gc.quadraticCurveTo(
				gis2fxX(xc), gis2fxY(yc),
				gis2fxX(x1), gis2fxY(y1));
	}
	...
	public void bezierCurveTo(double xc1, double yc1, double xc2, double yc2, double x1, double y1) {
		this.gc.bezierCurveTo(
				gis2fxX(xc1), gis2fxY(yc1),
				gis2fxX(xc2), gis2fxY(yc2),
				gis2fxX(x1), gis2fxY(y1));
	}
	...
	public void arc(double centerX, double centerY,
		...
		this.gc.arc(
				gis2fxX(centerX), gis2fxY(centerY),
				gis2fxSize(radiusX), gis2fxSize(radiusY),
				gis2fxAngle(Math.toDegrees(startAngle)), gis2fxSize(length));
	}
	...
	public void rect(double x, double y, double width, double height) {
		this.gc.rect(
				gis2fxX(x), gis2fxY(y),
				gis2fxSize(width), gis2fxSize(height));
	}
	...
	public void fillArc(double x, double y, double width, double height,
		...
		this.gc.fillArc(
				gis2fxX(x), gis2fxY(y),
				gis2fxSize(width), gis2fxSize(height),
				gis2fxAngle(Math.toDegrees(startAngle)),
				gis2fxAngle(Math.toDegrees(arcExtent)),
				closure);
	}
	...
	public void fillRoundRect(double x, double y, double width, double height, double arcWidth, double arcHeight) {
		this.gc.fillRoundRect(
				gis2fxX(x), gis2fxY(y),
				gis2fxSize(width), gis2fxSize(height),
				gis2fxSize(arcWidth), gis2fxSize(arcHeight));
	}
	...
	public void drawImage(Image img, double x, double y) {
		this.gc.drawImage(img, gis2fxX(x), gis2fxY(y));
	}
	...
	public void drawImage(Image img,
		...
		this.gc.drawImage(img,
				gis2fxX(sx), gis2fxY(sy),
				gis2fxSize(sw), gis2fxSize(sh),
				gis2fxX(dx), gis2fxY(dy),
				gis2fxSize(dw), gis2fxSize(dh));
	}
