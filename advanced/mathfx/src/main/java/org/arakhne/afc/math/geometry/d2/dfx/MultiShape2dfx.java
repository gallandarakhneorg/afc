 ...
 * @since 13.0
 */
public class MultiShape2dfx<T extends Shape2dfx<?>> extends AbstractShape2dfx<MultiShape2dfx<T>> implements 
MultiShape2afp<Shape2dfx<?>, MultiShape2dfx<T>, T, PathElement2dfx, Point2dfx, Vector2dfx, Rectangle2dfx> {
	...
	public ObjectProperty<Rectangle2dfx> boundingBoxProperty() {
		if (this.boundingBox == null) {
			...
			this.boundingBox.bind(Bindings.createObjectBinding(
					() -> {
						Rectangle2dfx box = getGeomFactory().newBox();
						Rectangle2dfx shapeBox = getGeomFactory().newBox();
	...
	public MultiShape2dfx<T> clone() {
		MultiShape2dfx<T> clone = super.clone();
		clone.elements = null;
		if (this.elements !=null) {
	...
	private static class InternalObservableList<T extends Shape2dfx<?>> extends ModifiableObservableListBase<T>
		...
		private void bind(T shape) {
			assert (shape != null);
			ObjectProperty<Rectangle2dfx> property = shape.boundingBoxProperty();
