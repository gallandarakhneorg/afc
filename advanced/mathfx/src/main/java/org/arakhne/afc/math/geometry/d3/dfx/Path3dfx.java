...
public class Path3dfx
	...
	public Point3dfx[] toPointArray(Transform3D transform) {
		...
		Point3dfx[] clone = new Point3dfx[n];
		if (n > 0) {
			if (transform == null || transform.isIdentity()) {
				Iterator<Double> iterator = this.coords.iterator();
				for(int i=0; i < n; ++i) {
				...
				Iterator<Double> iterator = this.coords.iterator();
				for(int i=0; i < n; ++i) {
					Point3dfx p = getGeomFactory().newPoint(iterator.next(), iterator.next(), iterator.next());
					transform.transform(p);
					clone[i] = p;
				}
			}
